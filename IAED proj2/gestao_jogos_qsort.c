...
Item * g_encontraEquipasMaisVitorias(Item * head_equipa, int contador, int n_equipas){
    Item * equipa = head_equipa;
    Item * menor_equipa = head_equipa;
    int maior_vitoria = 0;
    ...
    while(equipa){
        ...
        if (strcmp(menor_equipa->nome, equipa->nome) > 0){
            menor_equipa = equipa;
        }
        equipa = equipa->next;
    }
    ...
    if (head_equipa){
        ...
        qsort(head_equipa, n_equipas, sizeof(Item), comparador);
    }
